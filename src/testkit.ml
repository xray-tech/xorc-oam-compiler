...
module Serializer = struct
  ...
  let load_msg = function
    | M.List [M.Int 0; bc] ->
      Result.map (Serializer.load bc) (fun v ->
          Execute(v))
    | M.List ((M.Int 1)::(M.Int id)::tail) ->
      Result.map (Serializer.load_value (fun _ -> assert false) tail) (fun (v, _) ->
          Continue(id, v))
  ...
  let dump_res {Inter.Res.values; coeffects; killed} =
    ...
    M.List [
      M.List values';
      M.List (List.map coeffects dump_coeffect);
      M.List (List.map killed (fun x -> M.Int x))]
  ...
  let load_res v =
    ...
    with_return (fun r ->
        ...
        | M.List [M.List values; M.List coeffects; M.List killed] ->
          let values' = List.map values (function
              | M.List xs ->
                (match Serializer.load_value (fun _ -> assert false) xs with
                 | Ok((v, _)) -> v
                 | _ -> r.return err)
              | _ -> r.return err) in
          let coeffects' = List.map coeffects (function
              | M.List ((M.Int id)::xs) ->
                (match Serializer.load_value (fun _ -> assert false) xs with
                 | Ok((v, _)) -> (id, v)
                 | _ -> r.return err)
              | _ -> r.return err) in
          let killed' = List.map killed (function
              | M.Int v -> v
              | _ -> r.return err) in
          Ok { values = values';
               coeffects = coeffects';
